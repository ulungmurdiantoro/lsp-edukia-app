            ...
            @foreach($skemas[$key] as $s)
              <a href="{{ route('skema.show', $s['slug']) }}" class="scheme {{ $s['popular'] ? 'scheme--popular' : '' }}" data-cat="{{ $key }}">
                <div class="scheme-header" style="position:relative;overflow:hidden;">
                  ...
                  @if(!empty($s['gelar']))
                  <div class="gelar-pill"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1.5 3 3 6 3s6-1.5 6-3v-5"/></svg> Gelar Profesional: {{ $s['gelar'] }}</div>
                  @endif
                </div>
